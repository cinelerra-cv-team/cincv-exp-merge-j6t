...
// Char not found
		...
		if (gindex == 0 ||
			FT_Load_Char(freetype_face, glyph->char_code, FT_LOAD_RENDER))
		{
...
//printf("TitleMain::get_total_extents %d\n", ascent);
	...
	for(int i = 0; i < text_len; i++)
	{
		...
		int char_advance = get_char_advance(config.wtext[i], config.wtext[i + 1]);
...
//printf("TitleMain::overlay_mask 1\n");
	...
	{
		int fade_len = lroundf(config.fade_in *
			PluginVClient::project_frame_rate);
...
//printf("TitleMain::overlay_mask 1\n");
	...
	if(!EQUIV(config.fade_out, 0))
	{
		...
		int fade_len = lroundf(config.fade_out *
			PluginVClient::project_frame_rate);
